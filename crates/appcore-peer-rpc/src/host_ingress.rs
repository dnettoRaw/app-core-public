...
//! Admission before body collection and JSON decoding; health and manifest stay independent.
...
impl Ingress {
    pub(super) fn new(slots: Arc<Semaphore>, max_bytes: usize) -> Self {
        Self { slots, max_bytes }
    }
}
...
pub(super) async fn admit(
    ...
    let collected = receive(read, Duration::from_secs(10)).await;
    match collected {
        Ok(bytes) => {
...
mod tests {
    ...
    fn cancellation_releases_ingress_and_health_bypasses_saturation() {
        tokio::runtime::Builder::new_current_thread()
            ...
            .unwrap()
            .block_on(async {
                let ingress = Ingress::new(Arc::new(Semaphore::new(16)), 8);
