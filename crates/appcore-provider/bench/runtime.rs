...
//! Measures empty provider lookup through the typed registry.

use appcore_contracts::ProviderId;
use appcore_provider::{ProviderRegistry, ProviderRole};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let provider_id = ProviderId::new("bench-provider")?;
    let iterations = iterations(100_000);
    let started = benchmark_started();
    for _ in 0..iterations {
        black_box(registry.contains(ProviderRole::Storage, &provider_id));
    }
...
fn report(iterations: u64, total_ns: u128) {
    println!(
        "appcore-provider::empty_registry_lookup iterations={iterations} total_ns={total_ns} ns_per_iter={:.2}",
