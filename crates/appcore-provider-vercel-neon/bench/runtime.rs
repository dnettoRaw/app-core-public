...
//! Measures identity dispatch through the provider factory contract.

use appcore_provider::ProviderFactory;
...
fn main() {
    ...
    let iterations = iterations(500_000);
    let started = benchmark_started();
    for _ in 0..iterations {
        black_box(<VercelNeonControlPlaneFactory as ProviderFactory<
            SharedControlPlaneProvider,
        >>::provider_id(&factory));
    }
...
fn report(iterations: u64, total_ns: u128) {
    println!(
        "appcore-provider-vercel-neon::provider_identity iterations={iterations} total_ns={total_ns} ns_per_iter={:.2}",
